...
bool ImGui::SliderRoundScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	IM_ASSERT(*(const float*)v_min >= -FLT_MAX / 2.0f && *(const float*)v_max <= FLT_MAX / 2.0f);
	//const bool value_changed = SliderBehaviorT<float, float, float >(frame_bb, id, data_type, (float*)v, *(const float*)v_min, *(const float*)v_max, format, power, 0);
			

		const bool is_horizontal = (0 & ImGuiSliderFlags_Vertical) == 0;
		const bool is_decimal = (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double);
		const bool is_power = (power != 1.0f) && is_decimal;
		...
		float grab_sz = style.GrabMinSize;
		float v_range = (*(const float*)v_min < *(const float*)v_max ? *(const float*)v_max - *(const float*)v_min : *(const float*)v_min - *(const float*)v_max);
		if (!is_decimal && v_range >= 0)                                             // v_range < 0 may happen on integer overflows
			grab_sz = ImMax((float)(slider_sz / (v_range + 1)), style.GrabMinSize);  // For integer sliders: if possible have the grab size represent 1 unit
		grab_sz = ImMin(grab_sz, slider_sz);
		const float slider_usable_sz = slider_sz - grab_sz;
		...
		// For power curve sliders that cross over sign boundary we want the curve to be symmetric around 0.0f
		float linear_zero_pos;   // 0.0->1.0f
		if (is_power && *(const float*)v_min * *(const float*)v_max < 0.0f)
		{
			// Different sign
			const float linear_dist_min_to_0 = ImPow(*(const float*)v_min >= 0 ? (float)*(const float*)v_min : -(float)*(const float*)v_min, (float)1.0f / power);
			const float linear_dist_max_to_0 = ImPow(*(const float*)v_max >= 0 ? (float)*(const float*)v_max : -(float)*(const float*)v_max, (float)1.0f / power);
			linear_zero_pos = (float)(linear_dist_min_to_0 / (linear_dist_min_to_0 + linear_dist_max_to_0));
		}
		else
		{
			// Same sign
			linear_zero_pos = *(const float*)v_min < 0.0f ? 1.0f : 0.0f;
		}
		...
		if (g.ActiveId == id)
		{
			...
			else if (g.ActiveIdSource == ImGuiInputSource_Nav)
			{
				...
				{
					clicked_t = SliderBehaviorCalcRatioFromValue<float, float>(data_type, *(float*)v, *(const float*)v_min, *(const float*)v_max, power, linear_zero_pos);
					const int decimal_precision = is_decimal ? ImParseFormatPrecision(format, 3) : 0;
					if ((decimal_precision > 0) || is_power)
					{
						delta /= 100.0f;    // Gamepad/keyboard tweak speeds in % of slider bounds
			...
			if (set_new_value)
			{
				...
				if (is_power)
				{
					...
					if (clicked_t < linear_zero_pos)
					{
						...
						float a = 1.0f - (clicked_t / linear_zero_pos);
						a = ImPow(a, power);
						v_new = ImLerp(ImMin(*(const float*)v_max, (float)0), *(const float*)v_min, a);
					}
					else
					{
						// Positive: rescale to the positive range before powering
						...
						if (ImFabs(linear_zero_pos - 1.0f) > 1.e-6f)
							...
							a = clicked_t;
						a = ImPow(a, power);
						v_new = ImLerp(ImMax(*(const float*)v_min, (float)0), *(const float*)v_max, a);
					}
				}
				else
				{
					...
					if (is_decimal)
					{
						v_new = ImLerp(*(const float*)v_min, *(const float*)v_max, clicked_t);
					}
					else
					{
						// For integer values we want the clicking position to match the grab box so we round above
						// This code is carefully tuned to work with large values (e.g. high ranges of U64) while preserving this property..
						float v_new_off_f = (*(const float*)v_max - *(const float*)v_min) * clicked_t;
						float v_new_off_floor = (float)(v_new_off_f);
						float v_new_off_round = (float)(v_new_off_f + (float)0.5);
						if (!is_decimal && v_new_off_floor < v_new_off_round)
							v_new = *(const float*)v_min + v_new_off_round;
						else
							v_new = *(const float*)v_min + v_new_off_floor;
					}
				}

				// Round to user desired precision based on format string
				v_new = RoundScalarWithFormat<float, float>(format, data_type, v_new);
		...
		// Draw
		float grab_t = SliderBehaviorCalcRatioFromValue<float, float>(data_type, *(float*)v, *(const float*)v_min, *(const float*)v_min, power, linear_zero_pos);
		if (!is_horizontal)
			grab_t = 1.0f - grab_t;
		const float grab_pos = ImLerp(slider_usable_pos_min, slider_usable_pos_max, grab_t);
		ImRect grab_bb;
		if (is_horizontal)
			grab_bb = ImRect(grab_pos - grab_sz*0.5f, frame_bb.Min.y + grab_padding, grab_pos + grab_sz*0.5f, frame_bb.Max.y - grab_padding);
		else
			grab_bb = ImRect(frame_bb.Min.x + grab_padding, grab_pos - grab_sz*0.5f, frame_bb.Max.x - grab_padding, grab_pos + grab_sz*0.5f);
		window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
// Tip: pass an empty label (e.g. "##dummy") then you can use the space to draw other text or image.
// But you need to make sure the ID is unique, e.g. enclose calls in PushID/PopID or use ##unique_id.
bool ImGui::SelectableBorder(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
